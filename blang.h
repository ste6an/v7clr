...
public:
	...
	CString pName;
	//08h
	int Type; // 0 - переменная; 1 - процедура; 2 - функция; 3 - тело модуля
	//0Ch
	int BaseAddr;
	//10h
	int NParams;
	...
	void* vtable1;
	//18h
	CTypedPtrArray<CPtrArray,	CBLModuleProcVarDescr*> VarList; //массив ссылок на переменные модуля // CBLModuleProcVarDescr
	//2Ch
	void* vtable2;
	//30h
	CPtrArray LabelList; //массив ссылок на метки в модуле // CBLModuleProcLabelDescr
	//3Ch
	void* Flag8;
	//40h
	DWORD Flag9;		//(1 - Экспорт)
	//44h
	int Flag10;
	//48h
	int Flag11;
...
public:
	...
	void* vtable1;
	//04h
	CTypedPtrArray<CPtrArray, CBLModuleConstDescr*> ConstArray; // массив констант в модуле // CBLModuleConstDescr
	//18h
	void* vtable2;
	//1Ch
	CTypedPtrArray<CPtrArray, CCompileProcInfo*> ProcArray; // массив ссылок на процедуры модуля // CCompileProcInfo
	//30h
	CDWordArray PCode; // скомпилированный код
	//40h
	void* Flag3;
	//44h
	void* Flag4;
...
//	void* pVTable;
	...
	DWORD Flag_0x14;
	// 0x18
	CValue *m_pValue; // если Flag1 = 1, то адресуется массив размерностью dwFlag5
};
...
public:
	...
	DWORD val_3;
	//0Ch
	struct structSTACK* pStack; // массив - внутренний стек
	//10h
	DWORD val_5;
	//14h
	DWORD dwStackSize; // количество переменных в стеке
	//18h
	DWORD val_7;
	//1Ch
	DWORD val_8;
...
/*
class CExecutedModule // от АльФ-а
{
public:
     //00h
     class CBLModule* m_pModule;
...
public:
	...
	int  GetType(void)const;	//98 // вернуть 0 - функция, 1 - процедура, -1 - неудача
