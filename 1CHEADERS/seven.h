...
class IMPORT_1C CAddInContext : public CBLContext // artbear точный размер 34h
{};
...
public:
	...
	void  SetProfileFont(int,HFONT);	//1800
...
friend class CComponentClass;
	...
	int  ExecProc(unsigned int iStringResourceId,int bFlagNotShowErrorIfNotFoundProcFunc,class CValue *,int iParamsCount,class CValue * * params,int,int);	//592
...
public:
	...
	CCJDef* m_pCJDef;
	CObjID m_ObjID;
	CCJPeriodContext* m_pPeriod1;
	CCJPeriodContext* m_pPeriod2;
...
//!	static int  InsertNewFolders(struct IStorage *,class CMetaDataObj *,class CMap<class CMetaDataObj *,class CMetaDataObj *,long,long> *);	//1199
	...
	static BYTE*  ReadStream(LPCSTR szModulePath, long nID, ULONG& nReadLen, int nUnknown);	//1642
	...
	static BOOL TestCanExist(LPCSTR szModulePath, long nID, long& nStreamLen);	//1860
private:
...
protected:
	HBRUSH CtlColor(class CDC *,unsigned int);
	void OnEnable(int);
...
protected:
	...
	HBRUSH  CtlColor(class CDC *,unsigned int);	//520
public:
...
public:
	int  SetContent(HBITMAP);	//1741
	int  SetContent(HINSTANCE,char *);	//1742
...
public:
	...
	static HWND  TT7ChildFromPoint(HWND,struct tagPOINT);	//1852
	static int  TT7IsBrowser(HWND);	//1853
	static int  TT7IsComboBox(HWND,unsigned int);	//1854
	static int  TT7IsMaskEdit(HWND);	//1855
	static int  TT7IsStatic(HWND);	//1856
...
// АльФ - класс определен в frame.h
/*
...
public:
	...
	virtual HMENU  GetDefaultMenu(void);	//717
	...
	class CDocument *  FindDocument(char const *,int *nPos=NULL);	//646
	class CDocument *  GetActiveDocument(void);	//666
	class CDocument *  GetDEditDocument(void);	//708
	static HICON  GetFrameIcon(unsigned int);	//741
...
public: 
	...
	// 0x0110
	int		m_Unkn4; 
	int		m_Unkn5; //+ seven switch operand
	int		m_Unkn6; //+ tracer switch operand
	// 0x011C
...
IMPORT_1C class CString  __cdecl GetModuleFullName(long, LPCSTR szModulePath, int nID, class CMetaDataCont *);
...
IMPORT_1C char const * __cdecl GetMainProfileName(void);
