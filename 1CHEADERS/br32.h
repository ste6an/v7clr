...
public:
	...
	//int m_nGrowBy; // [0x58]
	// + ??h
	CObArray m_strArray; // массив содержимого ячеек // [0x5C]
	//void* vtable; // [0x5C]
...
//LONG    bottom; // [0x84]
	...
	DWORD m_param8; // [0xA8] // первая строка для перерисовки
	DWORD m_param9; // [0xAC] // последняя строка для перерисовки
	DWORD m_param10; // [0xB0]
	DWORD m_br_Font; // [0xB4] // Font
	DWORD m_param12; // [0xB8]
	DWORD m_param13; // [0xBC] // высота одной строки текста
	int m_SelRow; // [0xC0] // текущая строка
	int m_SelCol; // [0xC4] // текущая колонка
...
public:
	...
	class CColumn &  GetColumn(unsigned int ColumnID);	//127 // найти колонку по идентификатору колонки
	int  GetColumnIndex(unsigned int ColumnID);	//128// найти индекс колонку по идентификатору колонки
	class CColumn &  GetColumnN(int ColumnIndex);	//129 // найти колонку по номеру колонки в списке колонок
...
public:
	...
	void  HideColumn(unsigned int ColumnID);	//200
...
public:
	...
	void  ShowColumn(unsigned int ColumnID);	//407
...
public:
	...
	// перехват OnPain для раскрашивания таблицы
...
public:
	...
	//CGetDoc77* m_GetDoc1; // АльФ - указатель на атрибуты класса GetDoc7
	CGetDoc7* m_GetDoc1; // АльФ - указатель на атрибуты класса GetDoc7 artbear

	//CFormCtrl* m_FormCtrl;
	CGetField* m_FormCtrl; //АльФ
	int buf4[30];
};
...
public:
	...
	DWORD m_dwGetColumn2;
	 CGetColumn(unsigned int nID, char const * szID, int,int);	//20
