...
/*
// спец. класс, созданный АльФ-ом
// Это просто атрибуты класса GetDoc7, вынесенные в отдельный класс. Аппендикс :)

// kms
// трансляция смещений в атрибуты CGetDoc7
...
class IMPORT_1C CGetDoc7:public CGetDoc //CCmdTarget //CObject //CGetDoc //CGetDoc77 (АльФ)
{
...
//	CObArray mm_ctrlList;
	...
	// след. 2 поля m_pSavingWorkBookDoc и m_pSavingDocId используются в SetTableSourceForms
	...
	CControlList m_cControlList; //0x400h // данные - это CControlID*
	CEditDoc* m_pEditDoc; //[224]
	CFont m_cFont;
	CGetFieldsArray m_cGetFieldsArray1; // +42ch //массив ссылок на все поля (CGetField*) документа (CGetDoc7)
	CGetFieldsArray m_cGetFieldsArray2; // +444h //массив ссылок на все поля, кроме таких как текст, рамка, кнопка
	DWORD m_Flag237;
	
 	// artbear а это похоже массив указателей CGetField-ов которые нужно пересчитать
	CGetField** m_pArray_EvalStatics; // + 458h //массив ссылок на поля типа текст имеющие формулу
	...
	// artbear а это похоже еще один массив указателей CGetField-ов которые нужно пересчитать
	CGetField** m_pArray_BrowseEvalColumns; // + 46Сh
	...
	DWORD bufGetDoc7[0x01]; // 0x50c //оставляем для доступа к атрибутам наследников от CGetDoc7
/*
...
public:
	...
	// artbear
// 	DWORD m_object1; // 50h
// 	CGetField** m_pGetFieldsArray; // выделяется через new
// 	DWORD m_sizeFieldsArray; //58h
// 	DWORD m_dummy_5C; // максимальное количество чего-то 
// 	DWORD m_dummy_60;
	CGetFieldsArray m_GetFieldsArray;
	 
	CDWordArray m_DWordArray; //64h // в качестве элемента используется индекс в m_pDoc->m_CGetFieldsArray2

	// значения массива могут быть 1 или 2
	int* m_pArrayOfFlagsRefresh; // от 0 до m_sizeFieldsArray-1 // 78h
	// end
