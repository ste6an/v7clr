...
int CMyContextBase::GetNParams(int iMethodNum)const
{
	...
	if (MethDef.bIsUnLimParMethod)
	{
		// если передали меньше параметров, чем должно быть по умолчанию,
		// 1— должна выдать стандартную ошибку о количестве параметров
...
// artbear
  ...
  GetBaseInfo()->m_CountParametersInCallMethod = regEBX; // сохраним это значение
// end
